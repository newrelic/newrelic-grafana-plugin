...
func (m *mockNRDBExecutor) PerformNRQLQueryWithContext(ctx context.Context, accountID int, query nrdb.NRQL) (*nrdb.NRDBResultContainerMultiResultCustomized, error) {
	...
	// Create an equivalent multi-result container
	multiResult := &nrdb.NRDBResultContainerMultiResultCustomized{}
	if result != nil {
		multiResult.Metadata = result.Metadata
...
func TestCheckHealth_Validation(t *testing.T) {
	tests := []struct {
		name     string
		config   *models.PluginSettings
		...
		},
		{
			...
			config:   &models.PluginSettings{Secrets: nil},
			executor: &mockNRDBExecutor{},
			want: &backend.CheckHealthResult{
				Status:  backend.HealthStatusError,
				Message: "Plugin configuration validation failed: plugin secrets cannot be nil",
			},
			wantErr: false,
		},
		{
			...
			want: &backend.CheckHealthResult{
				Status:  backend.HealthStatusError,
				Message: "Plugin configuration validation failed: plugin settings cannot be nil",
			},
			wantErr: false,
		},
	}
